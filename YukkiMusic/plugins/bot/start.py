    ...
    for member in message.new_chat_members:
        try:
            ...
            if member.id == app.id:
                ...
                link_gr = f"@{message.chat.username}" if message.chat.username else await app.export_chat_invite_link(message.chat.id)
                await app.send_message(config.OWNER_ID[0],f"تـم اضـافة البـوت الي جـروب {message.chat.title} \n لينك الجروب {link_gr}")
